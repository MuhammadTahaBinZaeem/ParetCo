...
COPY server.js start.js preflight.js app_preflight.js round2_preflight.js round3_preflight.js round4_preflight.js round5_preflight.js round6_preflight.js round7_preflight.js native_feature_bootstrap.js ./
COPY ui/ ./ui/
...
# Initialize a writable 64-bit Wine prefix, run native-feature integration plus
# all seven app-layer reliability passes, then start production diagnostics.
# Native engine files are untouched.
CMD ["sh", "-c", "mkdir -p \"$WINEPREFIX\"; wineboot -u >/dev/null 2>&1 || true; exec node native_feature_bootstrap.js"]
